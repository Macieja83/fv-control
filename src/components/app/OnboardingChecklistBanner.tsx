...
import { fetchBillingSubscriptionState, type WorkspaceUsage } from '../../api/billingApi'
...
export function OnboardingChecklistBanner(props: {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setTenant(t)
      setWorkspaceUsage(sub?.workspace ?? null)
    } catch (e: unknown) {
  ...
  const steps = useMemo<Step[]>(() => {
    ...
    const subOk = workspaceUsage?.hasProEntitlement === true
    return [
    ...
    workspaceUsage?.hasProEntitlement,
    tenant?.nip,
