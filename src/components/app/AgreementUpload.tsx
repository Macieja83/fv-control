...
import { uploadAgreementFile } from '../../api/agreementsApi'
...
export function AgreementUpload({ onUploaded }: Props) {
  ...
  const handleFile = useCallback(
    async (file: File | undefined) => {
      if (!file) return
      setState('uploading')
      setMessage('Przesyłanie i odczyt umowy (AI) — może potrwać ok. minuty…')
      try {
        await uploadAgreementFile(file)
        setState('success')
        setMessage('Umowa zapisana i odczytana.')
        onUploaded()
